...
def heuristic(graph, current_station, end_station):
...
def a_star(graph, starting_station: str, ending_station: str):
    open_list = {starting_station}
    closed_list = set([])
    ...
    while len(open_list) > 0:
        ...
        for neighbour in open_list:
            if current is None or current_distance[neighbour] + heuristic(graph, neighbour, ending_station) < current_distance[current] + heuristic(graph, current, ending_station):
                current = neighbour
